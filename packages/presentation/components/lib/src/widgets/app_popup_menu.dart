import 'package:fonde_ui/fonde_ui.dart'
    show
        FondePopupMenu,
        FondePopupMenuItem,
        FondePopupMenuDivider,
        FondePopupMenuEntry,
        FondePopupMenuItemEntry,
        FondePopupMenuDividerEntry;
export 'package:fonde_ui/fonde_ui.dart'
    show
        FondePopupMenu,
        FondePopupMenuItem,
        FondePopupMenuDivider,
        FondePopupMenuEntry,
        FondePopupMenuItemEntry,
        FondePopupMenuDividerEntry;

typedef AppPopupMenu<T> = FondePopupMenu<T>;
typedef AppPopupMenuItem<T> = FondePopupMenuItem<T>;
typedef AppPopupMenuDivider = FondePopupMenuDivider;
typedef AppPopupMenuEntry<T> = FondePopupMenuEntry<T>;
typedef AppPopupMenuItemEntry<T> = FondePopupMenuItemEntry<T>;
typedef AppPopupMenuDividerEntry<T> = FondePopupMenuDividerEntry<T>;
