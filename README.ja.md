...
RinneGraph は、**ローカル・オフラインで動作するグラフベースの個人知識管理アプリ**です。細々とした情報のつながりを視覚化し、全体像の直感的な把握をサポートします。
...
一方、Neo4jやArangoDBのような本格的なグラフデータベースはサーバーが必要であり、開発者向けに設計されているため、個人的な用途には不向きです。また、macOS・Windows・モバイルでの動作をサポートしていません。RinneGraphはグラフデータベースの表現力を、**ローカル・オフライン・GUIファーストのデスクトップアプリ**として提供します。サーバーも面倒なセットアップも不要で、あなたのデータはあなただけのものです。

## 💡 機能

- **ローカルファースト・オフライン対応** — 完全にデバイス上で動作し、サーバーは不要
